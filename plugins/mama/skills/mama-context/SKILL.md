...
This skill provides **automatic background context injection** using MAMA's hook system. It runs silently and shows relevant past decisions when you:
- Submit a prompt (UserPromptSubmit hook)
- Read/Edit/Grep files (PreToolUse hook)
...
The skill is a **wrapper around MAMA hooks** that have already done the heavy lifting:

1. **UserPromptSubmit Hook** (`scripts/userpromptsubmit-hook.js`)
   - Triggers: Every user prompt
   - Similarity threshold: 75%
   - Token budget: 40 tokens (teaser format)
   - Output: Topic + similarity + time

2. **PreToolUse Hook** (`scripts/pretooluse-hook.js`)
   - Triggers: Before Read/Edit/Grep/Glob
   - Similarity threshold: 70% (lower for file ops)
   - Token budget: 300 tokens
   - Rate limit: 1 second between injections
   - Context: File-specific decisions
...
- PreToolUse: `scripts/pretooluse-hook.js`
- Shared core: `src/core/memory-inject.js`

**Performance:**
- Hook latency: <500ms (p95, AC requirement)
- Actual measured: ~100ms (10ms embedding + 50ms search + 40ms format)
...
1. **Lightweight:** 40 tokens, <500ms latency
2. **Transparent:** Always show tier status
3. **Non-intrusive:** Hints, not walls of text
4. **Opt-out:** User control via config
5. **Graceful degradation:** Tier 2 fallback
